...
class VaccinGenerate
{
    ...
    public function generateCalendar($patient, $dateNow)
    {
        ...
        $day_preg =  $dateNow;
        if($state ){
            switch ($type_patient->getTypePatientName()){
                case 'ENFANT':
                    $alls = $this->vaccinRepository->findVaccinByTYpe($type_patient, $state);
    ...
    public function generate_vaccin($patient, $birthday, $vaccinAll)
    {
        ...
        foreach ( $vaccinAll as $vacc){
            if($vacc != null){
                $datePriseInitiale = $vacc->getDatePrise();
                $type_ins = 'week';
                $diffIn = 1;
                if($datePriseInitiale != null){
                    $type_ins = explode(" ", $datePriseInitiale);
                }
                    ...
                    if ($getVAcc != null){
                        if($diffIn < intval(explode(" ", $getVAcc))) {
                            switch ($type_ins){
                                case 'week':
                            ...
                            $crnV->setDatePrise($date);
                            //$crnV->setRappelVaccin($date);

                        }elseif ($diffIn == intval(explode(" ", $getVAcc))){
                            if(!$date_now->isWeekend()){
                                $tomorrow = $date_now->addDay();
                                if($tomorrow->day != 6){
                                    $crnV->setDatePrise($tomorrow);
                                }else{
                                    $monday = $date_now->addDays(3);
                                    $crnV->setDatePrise($monday);
                                }
                            }else{
